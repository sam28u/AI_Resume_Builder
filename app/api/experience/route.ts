...
const experienceSchema = z.object({
  ...
  descriptionBullets: z.array(z.string().min(1)).default([]),
});
  ...
  try {
    ...
    const newExperience = await db
      ...
      .values({
        ...
        // Fix: Only convert to Date if it's a string, otherwise pass null
        endDate: parsedData.endDate ? new Date(parsedData.endDate) : null,
        descriptionBullets: parsedData.descriptionBullets,
      })
      .returning();
...
export async function PATCH(req: Request) {
  try {
    ...
    const parsedData = updateExperienceSchema.parse(body);
    ...
    if (rest.company !== undefined) {
      updateData.company = rest.company;
    }
    ...
    if (rest.startDate !== undefined) {
      updateData.startDate = new Date(rest.startDate);
    }
    ...
    if (rest.descriptionBullets !== undefined) {
      updateData.descriptionBullets = rest.descriptionBullets;
    }
...
export async function DELETE(req: Request) {
  try {
    ...
    const deletedExperience = await db
      ...
      .where(
        and(
          eq(experiences.id, parsedData.id),
